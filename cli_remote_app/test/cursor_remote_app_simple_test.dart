...
void main() {
  group('CursorRemoteApp Widget Tests', () {
    ...
    testWidgets('App displays empty logs message initially', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();

      // Should show placeholder text when no logs
  ...
  group('CursorRemoteScreen State Management Tests', () {
    ...
    testWidgets('Mode dropdown changes selected mode', (WidgetTester tester) async {
      ...
      // Find and tap the Terminal option if available
  ...
  group('Server URL Dialog Tests', () {
    ...
    testWidgets('Server URL dialog allows entering new URL', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();
      ...
      // Enter new URL
      await tester.enterText(textFields, 'http://new-server:4000');
      await tester.pumpAndSettle();

      // Verify the text was entered
      expect(find.text('http://new-server:4000'), findsOneWidget);
    });
  });
  ...
  group('Theme and Styling Tests', () {
    ...
    testWidgets('App has proper color scheme for terminal look', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();
      ...
      expect(colorScheme?.background, equals(Colors.black));
      expect(colorScheme?.primary, isA<Color>());
    });
  });

  group('SharedPreferences Tests', () {
    testWidgets('App loads with default server URL when no saved preferences', (WidgetTester tester) async {
    ...
    testWidgets('App loads saved server URL from SharedPreferences', (WidgetTester tester) async {
      // Mock SharedPreferences with saved URL
      SharedPreferences.setMockInitialValues({
  ...
  group('Error Handling Tests', () {
    ...
    testWidgets('App handles empty command input gracefully', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();
      ...
      // Try to submit empty text (should not crash)
  ...
  group('Accessibility Tests', () {
    ...
    testWidgets('Icons have proper tooltips', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();

      // Check that action buttons have tooltips
      final wifiButton = find.byIcon(Icons.wifi_off);
      final clearButton = find.byIcon(Icons.clear);
      final sendButton = find.byIcon(Icons.send);
  ...
  group('UI Layout Tests', () {
    ...
    testWidgets('Connection status bar displays correctly', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();

      // Check connection status bar elements
      expect(find.byIcon(Icons.wifi_off), findsOneWidget);
      expect(find.text('Disconnected'), findsOneWidget);
    });

    testWidgets('Mode selection area displays correctly', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();

      // Check mode selection elements
      expect(find.text('Mode:'), findsOneWidget);
      expect(find.byType(DropdownButton<String>), findsOneWidget);
    });

    testWidgets('Command input area displays correctly', (WidgetTester tester) async {
      await tester.pumpWidget(const CursorRemoteApp());
      await tester.pumpAndSettle();

      // Check command input area
      expect(find.byType(TextField), findsOneWidget);
      expect(find.byIcon(Icons.send), findsOneWidget);
    });
  });
